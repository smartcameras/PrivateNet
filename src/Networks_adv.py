...
class ResNet18_ARM___RAF(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        out = self.fc(x)
        return out, alpha, midFeature


class ResNet18_ARM_adv(nn.Module):
    def __init__(self, pretrained=True, num_classes=5, drop_rate=0):
        super(ResNet18_ARM_adv, self).__init__()
        self.drop_rate = drop_rate
        self.arrangement = nn.PixelShuffle(16)
        self.arm = Amend_raf()
        self.fc = nn.Linear(121, num_classes)
